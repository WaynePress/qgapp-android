...
public class MVChildFragmentAdapter extends BaseMultiItemQuickAdapter<VideoItemBean, BaseViewHolder> {
    public static final int TYPE_VIDEO = -0xff;
    public static final int TYPE_ADVERTISING = 1;

    public MVChildFragmentAdapter(List<VideoItemBean> data) {
        super(data);
        addItemType(TYPE_VIDEO, R.layout.item_mv_new);
    ...
    protected void convert(final BaseViewHolder holder, final VideoItemBean item) {
        ...
        GlideUtils.loadMVImagView(mContext, item.getPic(), imageView);

        holder.setText(R.id.tv_name, item.getTitle());

        switch (holder.getItemViewType()) {
            case TYPE_VIDEO:
                holder.setText(R.id.tv_update_time, item.getCreatetime());
