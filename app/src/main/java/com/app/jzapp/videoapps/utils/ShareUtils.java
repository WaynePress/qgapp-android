...
public class ShareUtils {
     ...
     * @param msgText  (分享内容)
     * @param type     (分享类型)
     * @param drawable (分享图片，若分享类型为ShareUtils.TEXT，则可以为null)
     */
...
//          Bitmap bt = bd.getBitmap();
    ...
    public void sharedQQ(Activity context, Bitmap bitmap) {
        Uri uri = Uri.parse(MediaStore.Images.Media.insertImage(
                context.getContentResolver(), BitmapFactory.decodeResource(context.getResources(), R.mipmap.ic_icon), null, null));
        Intent imageIntent = new Intent(Intent.ACTION_SEND);
        imageIntent.setPackage("com.tencent.mobileqq");
        imageIntent.setType("image/*");
        imageIntent.putExtra(Intent.EXTRA_STREAM, uri);
        imageIntent.putExtra(Intent.EXTRA_TEXT, "您的好友邀请您进入" + context.getString(R.string.app_name));
        imageIntent.putExtra(Intent.EXTRA_TITLE, context.getString(R.string.app_name));
